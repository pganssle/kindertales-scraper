...
@pytest.mark.asyncio
async def test_dynamic_scheduler_prioritizes_generated_media() -> None:
    """Media generated by running work precedes deeper ordinary queue work."""
    observed: list[str] = []
    queue = scheduler.DynamicScheduler(1, 1)
    ...
    async def root() -> None:
        ...
        queue.add(scheduler.Work("ordinary", action("ordinary"), depth=10))
        queue.add(scheduler.Work("media", action("media"), media=True))

    queue.add(scheduler.Work("root", root))
    results = await queue.run()
    assert observed == ["root", "media", "ordinary"]
    assert set(results) == {"root", "ordinary", "media"}
    with pytest.raises(scheduler.GraphError, match="after.*finished"):
        queue.add(scheduler.Work("late", action("late")))
    with pytest.raises(scheduler.GraphError, match="only run once"):
        await queue.run()


@pytest.mark.asyncio
async def test_dynamic_scheduler_limits_media_and_captures_failures() -> None:
    """Generated work retains concurrency limits and isolates action failures."""
    active_media = 0
    maximum_media = 0
    queue = scheduler.DynamicScheduler(3, 1)

    async def medium() -> None:
        nonlocal active_media, maximum_media
        active_media += 1
        maximum_media = max(maximum_media, active_media)
        await asyncio.sleep(0)
        active_media -= 1
    ...
    async def fail() -> None:
        msg = "generated failure"
        raise RuntimeError(msg)

    for index in range(3):
        queue.add(scheduler.Work(f"media-{index}", medium, media=True))
    queue.add(scheduler.Work("failure", fail))
    results = await queue.run()
    assert maximum_media == 1
    assert isinstance(results["failure"].error, RuntimeError)
...
@pytest.mark.parametrize(
    "work",
    [
        scheduler.Work("duplicate", lambda: asyncio.sleep(0)),
        scheduler.Work(
            "dependent",
...
def test_dynamic_scheduler_rejects_invalid_work(work: scheduler.Work) -> None:
    """Dynamic work has unique keys and expresses successors by enqueueing."""
    queue = scheduler.DynamicScheduler(1, 1)
    if not work.dependencies:
        queue.add(work)
    with pytest.raises(scheduler.GraphError, match="duplicate|dependencies"):
        queue.add(work)
...
@pytest.mark.asyncio
async def test_request_admission_prioritizes_waiting_media() -> None:
    """Media overtakes ordinary requests that are still awaiting admission."""
    limiter = ImmediateLimiter()
    requester = scheduler.Requester(policy(max_in_flight=1), limiter)
    observed: list[str] = []

    def sender(name: str) -> scheduler.Sender:
    ...
    ordinary = asyncio.create_task(requester.request(sender("ordinary")))
    medium = asyncio.create_task(requester.request(sender("media"), media=True))
    later = asyncio.create_task(requester.request(sender("later")))
    await asyncio.gather(ordinary, medium, later)
    assert observed == ["media", "ordinary", "later"]


@pytest.mark.asyncio
async def test_request_admission_propagates_limiter_failure() -> None:
    """A quota limiter failure reaches the request waiting for admission."""
    ...
    requester = scheduler.Requester(policy(), FailingLimiter())
    with pytest.raises(RuntimeError, match="limiter failed"):
